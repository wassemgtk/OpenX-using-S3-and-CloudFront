...
+---------------------------------------------------------------------------+
$Id: adRender.php 62345 2010-09-14 21:16:38Z chris.nutting $
*/
...
{
    $conf = $GLOBALS['_MAX']['CONF'];
    /*get random path from avaliable paths*/
    $images_paths = explode(",", $conf['webpath']['images']);
    $k = array_rand($images_paths);
    $webpath_images = $images_paths[$k];

    $imagesSSL_paths = explode(",", $conf['webpath']['imagesSSL']);
    $k_ssl = array_rand($imagesSSL_paths);
    $webpath_imagesSSL = $$imagesSSL_paths[$k_ssl];
    /////
    //return $GLOBALS['_MAX']['SSL_REQUEST'] ? 'https://' . $conf['webpath']['imagesSSL'] : 'http://' .  $conf['webpath']['images'];
    return $GLOBALS['_MAX']['SSL_REQUEST'] ? 'https://' . $webpath_imagesSSL : 'http://' .  $webpath_images;
}
