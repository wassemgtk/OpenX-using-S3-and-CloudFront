...
$table_prefix = $GLOBALS['_MAX']['CONF']['table']['prefix'];
		...
		$getcatres = mysql_query("SELECT * FROM oxm_keywords order by keyword") ;
		$num = mysql_num_rows($getcatres);
		$i = 1;
	    while($getcat = mysql_fetch_assoc($getcatres))
		{
			...
			if($i == 1):
				...
				$get_ele[0] = "-- Pick a Keyword --";
			
	
			endif;
				
			$get_ele[$i] = $getcat['keyword'];
			$i++;
			
	    	}


		$form->addElement('select', 'keyword_inline', 'Add Keywords for Inline ads',$get_ele);

    if(isset($zone['zoneid']) && $zone['zoneid']<>NULL)
	{


		$getcatres = mysql_query("SELECT * FROM oxm_keywords_zone  where zoneid=".$zone['zoneid']." ");
		$num = mysql_num_rows($getcatres);
		$i = 1;

	   	 while($getcat1 =mysql_fetch_assoc($getcatres))
		{

				
			$zone_keywords[$i] = $getcat1['keyword'];
			$i++;
		 ...
		 $form->addElement('select', 'keywordslist', 'Your Inline Keywords List',$zone_keywords);
...
$table_prefix = $GLOBALS['_MAX']['CONF']['table']['prefix'];
						...
						global $keywords;
							...
							$keyword_idex=$_POST['keyword_inline'];
							$i=0;


	
							if(isset($aFields['zoneid']))
							{

					$select=mysql_query("select * from oxm_keywords_zone where keyword='".$keywords[0][$keyword_idex]."' and zoneid=".$aFields['zoneid']);
						$count=mysql_num_rows($select);	

								if($count==0 && $_POST['keyword_inline']!=0 )
								{
								mysql_query("insert into oxm_keywords_zone 
								(keyword,zoneid) values('".$keywords[0][$keyword_idex]."',".$aFields['zoneid'].")") or die(mysql_error());
								}
							}
...
$table_prefix = $GLOBALS['_MAX']['CONF']['table']['prefix'];
						...
						global $keywords;
							...
							$select_key=mysql_fetch_row($select);
	
							if(isset($aFields['zoneid']))
							{
								if($select_key[0]==0 && $select_key[0]==NULL && $keyword_idex!=0 )
								{
								mysql_query("insert into oxm_keywords_zone 
								(keyword,zoneid) values('".$keywords[0][$keyword_idex]."',".$aFields['zoneid'].")") or die(mysql_error());
								}
